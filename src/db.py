...
def dashboard_page(user: dict) -> None:
    ...
    with st.expander("Filtros", expanded=True):
        ...
        with c3:
            ...
            start = st.date_input("Data inicial", value=min_date, format="DD/MM/YYYY")
        with c4:
            end = st.date_input("Data final", value=date.today(), format="DD/MM/YYYY")
    ...
    for date_col in ["evaluation_date", "Data", "data"]:
        if date_col in flat.columns:
            flat[date_col] = pd.to_datetime(flat[date_col], errors="coerce").dt.strftime("%d/%m/%Y")
    st.download_button(
...
def history_page(user: dict) -> None:
    ...
    summary = df[["id", "evaluation_date", "store", "supervisor", "manager", "weighted_score", "overall_status", "bonus_level", "manager_bonus"]].copy()
...
def action_plans_page(user: dict) -> None:
    ...
    with c3:
        deadline = st.date_input("Prazo", value=selected["deadline"].date() if pd.notna(selected["deadline"]) else date.today(), format="DD/MM/YYYY")
