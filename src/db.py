...
from .config import DB_PATH, DEFAULT_USERS, FORM_SECTIONS, STORES, UPLOAD_DIR, action_plan_recommendation
...
def init_db() -> None:
    ...
    _ensure_action_plan_columns(cur)
    ...
    for item in build_action_plan_items(evaluation_id, data):
        _exec(
            ...
            INSERT INTO action_plans (
                ...
                problem_text, impact_text, recommended_action, validation_criteria
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Aberto', ?, ?, ?, ?, ?)
            """,
            (
                ...
                item["problem_text"], item["impact_text"], item["recommended_action"], item["validation_criteria"],
            ),
        )
    conn.commit()
    conn.close()
    ...
    for section in FORM_SECTIONS:
        for q in section["questions"]:
            ...
            if score < 7 or (is_binary and score == 0):
                template = action_plan_recommendation(section["name"], key, q["label"])
                days = int(template.get("deadline_days") or default_action_deadline_days(score, is_binary))
                items.append({
...
def action_plans_df() -> pd.DataFrame:
    ...
    if not df.empty:
        ...
        for col in ["updated_at", "updated_by", "followup_log"]:
...
def update_action_plan(
    ...
    """Atualiza status, responsável, prazo e observações do plano.

    Também registra um histórico simples de acompanhamento em followup_log.
    Mantém compatibilidade com chamadas antigas que passavam só 5 argumentos.
    """
    now = datetime.now().isoformat(timespec="seconds")
    actor = updated_by or "Sistema"
    ...
    existing = None
    try:
        _exec(cur, "SELECT followup_log FROM action_plans WHERE id = ?", (plan_id,))
    ...
    previous_log = ""
    if existing:
        previous_log = str(existing.get("followup_log") or "")

    log_entry = f"[{now}] {actor}: status={status}; prazo={deadline}; responsável={responsible or 'Não definido'}"
    if followup_note and str(followup_note).strip():
        log_entry += f"; acompanhamento={str(followup_note).strip()}"
    new_log = (previous_log + "\n" + log_entry).strip() if previous_log else log_entry
    ...
    try:
        _exec(
            ...
            """
            UPDATE action_plans
               SET status = ?, responsible = ?, deadline = ?, notes = ?,
                   updated_at = ?, updated_by = ?, followup_log = ?
             WHERE id = ?
            """,
            (status, responsible, deadline, notes, now, actor, new_log, plan_id),
        )
    except Exception:
        # Fallback para bancos que ainda não tiveram as novas colunas aplicadas.
        _exec(
            cur,
            "UPDATE action_plans SET status = ?, responsible = ?, deadline = ?, notes = ? WHERE id = ?",
            (status, responsible, deadline, notes, plan_id),
        )
    conn.commit()
    conn.close()
